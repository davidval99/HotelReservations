    ...
    <div id="show-hotels" v-for="hotel in hotels" :key="hotel.id">
        ...
        <tr>
            ...
            <tr>
                ...
                <td>{{hotel.country}}</td>
                <td>{{hotel.region}}</td>
                <td><img :src=hotel.path align="right"></td>
            </tr>
        </table>
    </div>
  ...
  methods: {
   ...
   created() {
       ...
       localStorage.removeItem("selectedRoom");
        ...
        this.$http.get('http://localhost:8080/listAllRooms').then(function(data) {
            this.room = data.body;

        });

        this.$http.get('http://localhost:8080/listHotelsPuntuation').then(function(data){
